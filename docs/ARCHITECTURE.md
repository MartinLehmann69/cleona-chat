...
<!-- AUTO-GENERATED from Cleona_Chat_Architecture_v3_0.md (sha256:dc49dfccb470, 2026-06-25). -->
...
**Erasure is push-based**: the sender places fragments on send-failure, not on every send. Storage efficiency stays bounded. If erasure placement itself fails (zero DHT peers reachable at placement time), the serialized packet is parked in the persistent outbox (§5.1) and placement is re-attempted on the next `onNetworkChanged` edge — the sender does not silently drop unplaceable messages.
