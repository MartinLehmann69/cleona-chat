...
message NetworkPacket {
  ...
  repeated bytes visitedDeviceIds = 14;  // §5.3 anti-loop: each relay appends its own deviceId
  uint32  senderDataPort   = 15;  // sender's UDP listening port; 0 = unknown (legacy peer)
}
...
- **`visitedDeviceIds`**: §5.3 anti-loop list; each relay hop appends its own deviceId, the receiver drops a frame that already carries its own. Excluded from the signature scope because relays mutate it.
- **`senderDataPort`** (V3.1.159): the port the sender **actually listens on**. Receivers MUST prefer it over the observed UDP source port **only when the source IP is private** (RFC1918 / ULA / link-local). Behind NAT or CGNAT the observed port stays authoritative — it is the only port for which a hole-punch return path exists. The field lies **inside** the Outer-Device-Sig scope (only `ttl`, `hopCount`, `visitedDeviceIds`, the sigs and `networkTag` are excluded), so a relay can neither forge nor strip it undetected. `0` means "not advertised" and receivers then fall back to the observed port.

  Rationale: the UDP source port is not a reliable statement about where a peer listens. The Windows send path binds its socket ephemerally (`localPort: 0`) to keep the Dart receive socket the sole owner of the data port, so its packets never carry the listening port in the UDP header. Receivers used to learn that ephemeral port as a reachable address; measured on one peer: 15 stored addresses, 14 of them dead, ~900 accumulated failed send attempts. LAN discovery never had this problem because it carries the port in the payload — `senderDataPort` gives the V3 data path the same guarantee.
...
message InfrastructureFrame {
  ...
  bytes       inReplyTo          = 8;   // 16 bytes — echo of the request's messageId
}
```

**RPC correlation (V3.1.159)**: every **response** row in the selector list below MUST echo the request's `messageId` in `inReplyTo`. Receivers correlate primarily on that field; when it is empty they fall back to the `(peer, requestType)` tuple. The fallback is the rolling-upgrade path for peers that predate V3.1.159 and stays until every node sets the field.

The tuple alone is ambiguous by construction: two concurrent lookups of the same type to the same peer — two `DHT_FIND_NODE` for different targets is Kademlia routine — collide on it, and the first response to arrive completes whichever request happens to be registered. In S280 exactly that swapped two identity lookups 68 ms apart and overwrote a contact's trust anchor. Requests leave `inReplyTo` empty.
...
- **On request**, attached to the `DHT_PONG` answering a BOOT-path `DHT_PING` that carries `want_kem_record` (§5.10.2). This is the only channel that works when the initiator knows nothing but an `IP:port` — the situation Manual Peer Entry and QR seed-peer bootstrap create, where the DHT lookup that would yield the record is itself not yet reachable. The record is self-authenticating (§4.3), so attaching it grants the initiator nothing it could not have fetched from the DHT; it only removes the ordering constraint that the DHT must be reachable first. **Opt-in, not unconditional**: the record is ≈1.4 KB, which pushes the PONG past the 1200-byte fragmentation threshold — see §5.10.2 for why that cost is only paid where it buys something
...
- **Mobile/desktop daemons**: a random port in `[10000, 64999]` **excluding the well-known discovery port 41338** (§4.5.2 invariant), fixed on first start and persisted in `identities.json`. It remains stable across daemon restarts — important for NAT hole punching and for stored routing-table entries. Manual port changes via Settings are persisted to the same file (`IdentityManager.updatePort()`) so they survive restarts; the IPC handler rejects 41338 for the same reason, and an identity that already carries it from an earlier install re-draws on load.
...
**Eviction**: oldest-entry-first eviction when a bucket exceeds k=200 — the oldest entry is probed with 3× Cleona-PING (10ms/500ms inter-probe delay, 2s PONG timeout each) before eviction; only if all 3 probes fail is the entry replaced by the new candidate (stale threshold: 72 hours). If the evicted peer responds to the probe it is re-added to the bucket. With k=200 and realistic mesh sizes (10–500 nodes), eviction is a safety net that rarely triggers, not a load-shedding mechanism. Note: periodic self-broadcasts (§5.11 `_dvSafetyNetExchange`, hourly) refresh `lastSeen` for all peers — only truly departed nodes (offline >72h AND unresponsive to 3-probe) become eviction candidates.
...
**Response binding (mandatory):** a record returned for a lookup MUST carry the requested identifier — `AuthManifest.userId`, `LivenessRecord.(userId, deviceNodeId)` and `DeviceKemRecord.(userId, deviceId)` are checked against the lookup key **before any cryptographic verification**. `verifySelfCertified()` binds the embedded key to `manifest.userId`, i.e. to itself; without this check a foreign but self-consistent manifest passes verification and is only rejected by the contact-continuity check below, which then triggers the self-heal path with a record that never belonged to the contact. Rationale (2026-07-27): a concurrent-lookup collision in `DhtRpc` (pending table keyed by `(peer, requestType)` only) cross-delivered one identity's AuthManifest to another identity's lookup; the missing binding turned that into a contact-key overwrite. For the Liveness path the binding is independently necessary — the authorized-device filter only proves manifest membership, not that the response answers *this* lookup key.
...
- **Contact IPv6 address refresh via PEER_LIST_WANT (V3.1.148)**: on each processed `ROUTE_UPDATE` (when `result.changed`), the node scans contact devices already in the routing table that lack a recent global IPv6 address (< 30 min `lastSuccess`). For up to 5 such devices (5-min cooldown per device), a `PEER_LIST_WANT` is sent to the ROUTE_UPDATE's sender — requesting fresh peer metadata including IPv6 addresses that the contact may have gained since first discovery. This closes a gap where contacts behind CGNAT (IPv4-only at discovery time) never acquired IPv6 relay paths even after IPv6 became available on their network, because the existing DV→K-bucket WANT mechanism skips peers already present in the routing table.
...
**Default-Gateway Selection (V3.1)**: `updateDefaultGateway()` scores each DV neighbor by six criteria in strict priority order:
1. **usable** — the neighbor is admission-verified (§13.1.2) **and** currently has at least one address that is neither in backoff nor unreachable from the current network. Both conditions are re-checked by `sendToDevice` immediately before the gateway is used, so a neighbor failing either is a gateway in name only: every fall-through to it is skipped, and for fire-and-forget message types the cascade then ends with nothing sent. This is a **sort criterion, not a hard filter** — if no neighbor is usable, one is still elected so the gateway tier does not vanish entirely. The incumbent gateway wins ties, which is required rather than cosmetic: address backoff flips on a five-second granularity while `updateDefaultGateway()` runs on every changed route update, so without stickiness the election would oscillate several times per minute in a large mesh. Note that the selection is otherwise blind to physical reachability, and the skip on the send path produces no ACK timeout — so nothing demotes a permanently unreachable gateway on its own (`markRouteDown` needs timeouts, `evictStalePeers` needs zero alive routes, and startup re-elects from the persisted topology). Without this criterion the state is self-preserving.
2. **relay-confirmed** — neighbors through which a DELIVERY_RECEIPT has been received beat unconfirmed neighbors unconditionally.
3. **unique destination coverage** — count of destinations reachable ONLY through this neighbor. A neighbor that is the sole relay to mobile/CGNAT devices (e.g. Bootstrap) scores higher than a LAN peer with many routes but no exclusive destinations. Prevents high-route-count LAN neighbors from shadowing the only relay path to hard-to-reach devices.
4. **route count** — total alive destinations reachable via this neighbor.
5. **average cost** — lower is better.
6. **recency** — most recent `lastConfirmed` timestamp breaks ties.
...
`lastSuccess`, `lastAttempt` and `consecutiveFailures` are **persisted with the routing table** (V3.1.159). Without persistence every daemon restart resets both timestamps to null; the prune predicate can then never fire again, dead addresses accumulate without bound, and the failure backoff restarts at zero so known-dead addresses are re-sprayed at full rate. Records written before V3.1.159 come back without the timestamps — an address that carries failures but never a single success is treated as dead and pruned on load, which drains that backlog once.
...
|---|---|---|---|
| **1 — Stored peers** | Two phases: **(A) Contact-First** — probe ALL known contact device addresses (from `resolveContactDeviceIds` + routing table), no cap. These are the peers the user actually communicates with and the most valuable mesh entry-points. **(B) Diverse Mesh** — probe up to 5 additional non-contact stored peers, sorted by stability tier (§4.9.2) then lastSeen recency, for mesh diversity. One `DHT_PING` per peer on all reachable addresses, `PEER_LIST_WANT` to top 3 responders, 5 s timeout. V3.1.149: Contact-First eliminates the Bootstrap-SPOF discovered in S230 — contacts with known WAN addresses (public IPv6, NAT-punched IPv4) are reached directly without Bootstrap. | Always (if routing table non-empty) | N+5 PINGs (N = contact devices) + 1 WANT + 1 PUSH |
...
**What the shim does.** A single C source file under `native/cleona_net/` (version 1.2.0) exposes six functions: open an IPv4 UDP socket bound to a given local port, open an IPv6 UDP socket (`cleona_udp_open6`, `AF_INET6` with `IPV6_V6ONLY`), configure send and receive buffer sizes, send one datagram to an IPv4 destination, send one datagram to an IPv6 destination (`cleona_udp_send6`, with automatic scope-id suffix stripping), and close the socket. The Dart side wraps each function in `dart:ffi` bindings under `lib/core/network/native_udp_sender.dart` — `NativeUdpSender` for IPv4 and `NativeUdpSender6` for IPv6. `LocalDiscovery` holds one `NativeUdpSender` instance for the lifetime of the daemon, opened against the well-known discovery port 41338. `Transport` holds an additional `NativeUdpSender6` on Windows for IPv6 sends on the main data port (see below). The shim is **send-only** — no recv, no select, no epoll. Receive remains in Dart's `RawDatagramSocket.listen()` callback as before, because the receive path has no observable defect on any platform we tested.

**IPv6 native sender and Dart socket.send() safety net (V3.1.145).** Dart's IOCP-based `RawDatagramSocket.send()` on Windows can crash the Dart VM (`GetStackPointerForStackBounds failed`) when sending to IPv6 destinations that have no valid route — observed on Windows VMs with IPv6 enabled at the OS level but no IPv6 routing configured. The crash occurs below any Dart-level try-catch. `Transport` on Windows now uses `NativeUdpSender6` (backed by `cleona_udp_send6` → synchronous `WSASendTo` with non-blocking retry, no IOCP involvement) for all IPv6 sends. As defense-in-depth, the Dart `socket.send()` fallback path (used when no native sender covers the address family) is wrapped in a try-catch that logs the exception and returns 0 instead of letting it propagate and kill the isolate. This safety net is active on all platforms, not just Windows.
...
**Manual Peer Entry — what it can and cannot do.** The user supplies an `IP:port` and nothing else: no DeviceID, no keys. First contact is therefore a plaintext discovery probe (38 B, `CLEO` magic) sent to both the well-known discovery port and the port the user typed. A `DHT_PING` cannot be emitted at that moment — `InfrastructureFrameV3.recipientDeviceId` has no value to carry — so the entry is asynchronous by nature: nothing is added to the routing table by the call itself. Once the target answers with its own discovery packet, the receive handler registers it (creating the `PeerInfo` if the device is new) and the ping that immediately follows in the same handler does go out. The entry consequently works whenever the target is reachable on either of those two ports, and does nothing at all when it is not — behind CGNAT without a pinhole it cannot succeed by this mechanism and needs the rendezvous path (§4.11).

One limitation outlives the discovery round-trip: the peer is now pingable, but every **KEM-path** message to it (DHT_STORE, FRAGMENT_STORE, PEER_STORE, RELAY_*) is still dropped sender-side for a missing Device-KEM-PK, because that record is otherwise distributed through the 2D-DHT alone (§3.5b) — which a peer known by address has not yet reached. `want_kem_record` on the ping closes exactly this gap (§5.10.2).
...
**Resulting invariant: `nodePort != discoveryPort`.** Moving both discovery sockets onto the fixed port 41338 solves the collision only as long as no node's *data* port happens to be 41338. If it is, the same split reappears with the roles inverted: `Transport` and `LocalDiscovery`/`MulticastDiscovery` all bind 41338, all with `SO_REUSEADDR`, so the bind succeeds silently and the kernel splits inbound traffic between them. V3 packets landing on a discovery socket die at the 38-byte length check; discovery frames landing on the transport socket die at the HMAC check. Sending is unaffected, so such a node keeps emitting valid V3 traffic, is registered as a healthy DV neighbour everywhere, and drives up its peers' address success counters — while its own reception is non-deterministically lossy. Worse, it advertises 41338 as its data port in every discovery frame, so every peer stores that address, gossips it onward and persists it; the poisoned entry then competes with the node's real address on equal footing. Port assignment (§4.5.1) therefore excludes 41338 for both the random draw and manual changes, and a node that already carries the port from an earlier install re-draws it on identity load. Note that this is an exclusion, not a new fixed port: the data port stays random per node, which is what keeps Cleona from being blockable by a single port rule. The exclusion in fact removes a fingerprinting surface — a node whose data port equals the publicly documented discovery port identifies itself as Cleona to any scanner.

**`SO_REUSEADDR` on the transport sockets is load-bearing — do not remove it.** The obvious-looking fix for the collision above is to drop `SO_REUSEADDR` from the transport binds so that a duplicate bind fails loudly with `EADDRINUSE` instead of splitting silently. That breaks IPv6 outright. `Transport.start()` binds `anyIPv4:P` and `anyIPv6:P`, and with `net.ipv6.bindv6only = 0` (the Linux default) the IPv6 wildcard socket also covers IPv4 — the two only coexist on one port number because **both** set `SO_REUSEADDR`. Measured on Linux, all three variants fail the IPv6 bind with `errno 98`: both sockets without it, IPv4 without / IPv6 with, and IPv4 with / IPv6 without. Only the current both-with configuration binds successfully.

What *can* be tightened is the availability probe in `Transport.rebind()`. It binds the candidate port before tearing down the running transport, and with Dart's default `reuseAddress: true` it is not an availability test at all: `SO_REUSEADDR` lets it bind alongside a socket that already owns the port, so it reports "free" for a port that is taken — including our own discovery socket on 41338. The probe therefore binds with `reuseAddress: false` (measured: against a holder with `SO_REUSEADDR`, `true` succeeds and `false` correctly fails), which closes the runtime path into the collision that §4.5.1's port-assignment rules close at creation time.
...
**Bootstrap resilience**: stored routing-table entries are the primary discovery source (Tier 1 of the §4.5 cascade). The daemon probes **contact device addresses first** (all known, no cap — V3.1.149) then non-contact stored peers (max 5) — only if all probes fail does it escalate to LAN discovery (Tier 2), then bootstrap + rendezvous in parallel (Tier 3+3b), then subnet scan (Tier 4). A fresh install with an empty routing table starts at Tier 2 directly.
...
4. Soft-reset of the routing table: stale routes get a mark-stale flag (+5 cost penalty), 120s deadline; after deadline, remaining stale routes are probed with 3× Cleona-PING before removal.
...
**Who publishes**: every node that has at least one externally reachable address after the address filter (§4.11.7) — public IPv4 via STUN/UPnP or global IPv6. This naturally includes bootstrap nodes (always publicly reachable), port-forwarded desktops, and IPv6-capable mobile nodes. Infrastructure daemons (`cleona-daemon --bootstrap`) participate without a user identity — only the network secret and the device's own nodeId are needed.

**Who resolves**: any node during the discovery cascade (§4.5). Tier 3b runs **in parallel with Tier 3** (bootstrap probes — V3.1.149), not after it. Within Tier 3b, the infra-resolve runs in **parallel** with the contact-resolve (§4.11.4).
...
**End-to-end cold-start example**: Alice's phone was offline over the weekend. Monday morning: Tier 1 contact-first probes find Bob's phone on its public IPv6 → mesh entry in <3s (V3.1.149). If all Tier 1 probes fail: Tier 2 (LAN) fires, then Tier 3 (bootstrap) + Tier 3b (Nostr) fire in parallel: (A) contact-resolve for her contacts → succeeds if any contact has a public IP and published; (B) infra-resolve → finds Bootstrap's current public IP → connects → enters the mesh → DV routing delivers the rest.
...
Tier 3b runs **in parallel with Tier 3** (V3.1.149) but remains part of the cascade (not a separate timer): it inherits the same edge-trigger discipline as §5.1 (no timer-based retry). Decoupling into an independent periodic resolve was rejected — the cascade-integrated approach keeps traffic O(1) per startup. Fragment-NACK (CFNK) is orthogonal and analysed separately.
...
**Relay-forward route exclusion (V3.1.148)**: when `sendToDevice` routes a relay-forwarded packet (`isRelay=true`), it excludes as next-hop candidates: (1) the IP-resolved relay sender (`excludeNextHopHex`, existing), (2) the packet's originator (`packet.senderDeviceId`), and (3) all nodes in `packet.visitedDeviceIds`. Previously only (1) was enforced — but behind CGNAT, `_resolveDeviceHexFromAddress` returns null (many peers share the same public IP), leaving `excludeNextHopHex` empty. This allowed silent false-success: the relay node's `sendToDevice` cascade reported `ok=true` (kernel accepted the UDP send) but routed the packet back to the originator or a visited node — which either dropped it (originator-check) or created a bounce that consumed TTL without delivery. The fix ensures relay-forward never backtracks regardless of IP-resolution ambiguity.
...
**Confidentiality of a share in transit — a hard constraint on §2.3.5.** On the *return* leg the guardian answers with a `GuardianRestoreResponse` that carries the **raw** Shamir share of the 32-byte master seed, with no inner encryption layer of its own, and it travels as `FRAGMENT_STORE`. Its only confidentiality layer is therefore the frame-level Device-KEM encap of the `InfrastructureFrameV3`. That makes the §2.3.5 path assignment of `FRAGMENT_STORE` **security-relevant rather than a performance choice**: moving the type to the BOOT path strips its sole protection, and because relay hops forward packets unchanged — they only decrement `ttl`/`hopCount` — every hop and every passive observer holding the network secret (that is, every official build) would read the share. Three of five reconstruct the seed.

This is not hypothetical. Commit `30485b6d` (2026-05-21), an identity/QR bugfix spanning eight files, moved fragment and peer storage onto the BOOT path as a side effect in order to dodge the Device-KEM cold-start race, and the guardian consequence went unnoticed for two months because the coupling was documented nowhere. Should a future change genuinely require fragment storage on the BOOT path, the guardian payload must receive its own encryption layer **first**. The cold-start race itself no longer justifies such a move: `want_kem_record` (§5.10.2) supplies the missing `DeviceKemRecord` within one round-trip.
...
**ContactSeed readiness (V3.1.116, updated V3.1.151).** A ContactSeed is issued as soon as the node knows **at least one externally reachable own address** — a STUN/UPnP-discovered public IPv4 or a global IPv6 (not link-local, not ULA) — **or has at least one session-confirmed peer** (any address family). The session-confirmed-peer gate covers both LAN-only nodes and CGNAT/DS-Lite nodes that have confirmed WAN peers but whose STUN port-probe was not reciprocated (no `publicIp`, no `globalIpv6`). The seed-peer selector (Pass 1/2) already prefers WAN-reachable peers as relay candidates for the ContactSeed, so the `isReady` gate must not block seeds whose only confirmed peers have WAN addresses. Delivery paths for such a seed are direct send to the `a=` addresses, relay via the `s=` seed peers, plus the First-Contact Rendezvous (§4.11.10). (Previously an additional `_hasOnlyLanPeers` check blocked CGNAT nodes that had WAN-confirmed peers but no public IP, creating a permanent ContactSeed loading state.)
...
**Android must additionally set `flutter_deeplinking_enabled=false`** (`AndroidManifest.xml`, `MainActivity` meta-data). Flutter enables route-based deep linking automatically as soon as a `VIEW`+`BROWSABLE` intent-filter is declared (Flutter 3.24+). Since the app has no named routes — `MaterialApp` declares only `home:`, no `routes`/`onGenerateRoute`/`onUnknownRoute` — the engine's `pushRouteInformation` lands in `_onUnknownRoute` and raises an uncaught `FlutterError`, which the global `FlutterError.onError` handler escalates into a modal crash-consent dialog (`barrierDismissible: false`). Every `cleona://` link — ContactSeed *and* channel — then blocks the UI behind that modal instead of opening the intended flow. The flag disables only the engine path; Cleona's own handling (`MainActivity.handleDeepLinkIntent` → `pendingDeepLink` → `MethodChannel` drain on resume) does not use the navigation channel and is unaffected. Do not remove the flag without first giving `MaterialApp` a route generator.
...
- **Log (filtered):** Up to 200 lines from the main ring, excluding transport DEBUG noise and all TRACE lines. TRACE is the level for per-destination send-path decisions whose content is constant for a whole round (default-gateway skip, fire-and-forget cut-off, relay cooldown); they stay in the log file for field RCA but never enter the report ring, where a node with a few hundred peers would otherwise fill the entire 200-line window in milliseconds and reduce the report to a few seconds of history. Covers a much longer time window than the unfiltered 200-line tail.
...
**UDP receive-health self-probe.** `checkReceiveHealth()` (called by the 5s daemon heartbeat) sends a 4-byte raw probe (`0x43 0x50 0x52 0x42` = "CPRB") to 127.0.0.1 on the node's own port. The probe is intentionally too short for V3 outer-frame parsing — it serves solely to set `_lastUdpReceiveMs` in the `onUdpEvent` handler *before* any HMAC/parse step, confirming that the Dart `RawDatagramSocket` listen callback is alive. If no UDP event (including the self-probe) fires within 30 seconds, the socket is considered dead and `reconnectUdpSockets()` triggers (with its built-in 3× bind retry and deferred-recovery fallback on total failure). The probe's parse failure is suppressed in the V3 parser (loopback + ≤4 bytes → no "HMAC fail" log line) to avoid misleading log noise. On iOS, an additional `recvPeek()` path detects EBADF socket death (see §24.2).
...
|-------|----------|---------|
| DHT update-manifest poll | T+30s, T+5min retry, then 6 hours | Refresh the cached signed update manifest from DHT (§19.5.5). If zero fragments at T+30s, single retry at T+5min before 6h cycle |
...
The update-manifest poll runs an initial check at T+30s after startup. If zero fragments are returned, a single retry fires at T+5min. After that, the 6-hour periodic cycle takes over. A delayed startup push (`_pushManifestToPeers` at T+90s) actively distributes a cached manifest newer than the running version to all k-bucket peers. Users with stale caches see any required UpdateRequiredScreen on the next start after the cache refreshes (see §19.5.7 for the full UX flow).
...
1. **Platform events:** `connectivity_plus` on Flutter (GUI), periodic `NetworkInterface` polling on daemons.
...
**Async Crypto Pipeline:** The full inner send pipeline (User-Sign → zstd → KEM-Encrypt → PoW) runs in a single background isolate via `V3FrameCodec.buildAndEncryptInnerWithPowAsync()`. The isolate loads libsodium and liboqs independently. Two performance optimizations reduce per-message overhead: (1) **Native PoW loop (`libcleona_pow`):** the SHA-256 hashcash iteration runs in C (`cleona_pow_find_nonce`), eliminating ~1M Dart↔C FFI transitions per message. `libcleona_pow` is deployed on Linux and macOS (dynamic library), iOS (statically linked via XCFramework + `DynamicLibrary.process()`) and Android (`libcleona_pow.so` via jniLibs + `DynamicLibrary.open()`). **Windows currently runs the Dart fallback:** `native/cleona_pow/CMakeLists.txt:41` resolves libsodium via `find_library(SODIUM_LIB …)`, which needs an MSVC import library (`.lib`); the repository ships only the runtime `libsodium.dll` for Windows, so the CMake configure step fails. This was invisible until 2026-07-27 because `win-deploy.sh` invoked bare `cmake` (not on PATH on the build VM — it ships inside the VS Build Tools) and swallowed the failure into a `|| warn`. The Dart-side iteration loop serves as transparent fallback if the native library fails to load. (2) **OQS context caching:** the `OQS_SIG` and `OQS_KEM` context handles for ML-DSA-65 and ML-KEM-768 are allocated once per isolate and reused across calls, eliminating per-operation `OQS_*_new`/`OQS_*_free` overhead. Pre-hashing (§13.1.2) ensures PoW iteration time is constant regardless of payload size. The UI shows the message immediately with "sending" status (hourglass).
...
**Windows Desktop**:
- Distribution: Inno Setup installer (`-setup.exe`), installs to `%LOCALAPPDATA%\Cleona Chat\` (no admin privileges required). Creates Start Menu entry, optional Desktop shortcut, optional daemon autostart via Registry (`HKCU\Software\Microsoft\Windows\CurrentVersion\Run`). Uninstaller stops daemon+GUI and removes the Registry entry. Subsequent updates are delivered via the In-App-Update ZIP mechanism (§19.6), not a new installer run.
...
**Foreground service (canonical background-delivery path):** `CleonaForegroundService` boots as type `specialUse` (API 34+, `PROPERTY_SPECIAL_USE_FGS_SUBTYPE = "persistent_p2p_messaging_daemon"`) or `dataSync` (pre-34 fallback) and keeps the UDP socket alive for incoming message and call delivery. This is the **only** mechanism Cleona uses for background delivery on Android — push wake-up via FCM, UnifiedPush, WebPush or per-peer rotation has been architecturally evaluated and rejected (see §12.4 / ADR "Push Wake-Up Rejected"). During an active voice/video call the service is runtime-promoted to `dataSync|microphone` via the 3-arg `startForeground()` overload, after `RECORD_AUDIO` has been granted; it is demoted back to `dataSync` when the call ends. The manifest declares `foregroundServiceType="specialUse|microphone|dataSync"` so both the boot type (`specialUse` on API 34+, `dataSync` on pre-34) and the runtime call-promotion are permitted; calling `startForeground()` with the 2-arg overload at boot would implicitly inherit the `microphone` type and crash on freshly installed apps that have never granted `RECORD_AUDIO`. The persistent notification shows live connection status, e.g. "Connected — X peers", "Mobile — X peers", "Searching for peers…", or "Offline — no network" (actual strings come from i18n in 34 languages — see §17). Updated dynamically via MethodChannel (`updateServiceNotification`) on every state change, with dedup to avoid redundant updates. **Android 14 dismiss restore (V3.1.159):** Android 14+ (One UI 6.x) allows users to dismiss `setOngoing(true)` FGS notifications by swiping. The process and service singleton survive, but the notification is gone. Restore path: `MainActivity.ensureForegroundService` calls `startForegroundService` on every `onResume` → `onStartCommand` re-issues `startForeground` with a notification built from cached title/text (volatile companion vars written by `updateNotification()`), not a hardcoded default. On the Dart side, `_lastNotificationText` is reset to `''` at `AppLifecycleState.resumed` so the next `updateServiceNotification` call is not filtered by dedup. Channel importance: `IMPORTANCE_LOW` (no sound, no vibration, no badge).
...
- `MainActivity.ensureForegroundService` runs in `onCreate` + `onResume` and always calls `startForegroundService` (no `instance != null` guard). Required for the Android-14 dismiss-restore path: the service singleton survives notification dismissal, so the old guard prevented the `onStartCommand` re-issue that restores it. The OS deduplicates redundant intents cheaply.
...
**Not published:** Private keys, test infrastructure (`test/`, `scripts/vm/`), internal docs (`CLAUDE.md`, `HANDOVER.md`, `BUGFIX_*.md`), internal tooling (`init_profile.dart`), debug builds, VM credentials.
...
3. If true, `UpdateRequiredScreen` is rendered as the initial route, before the normal `MaterialApp` shell. The screen shows:
   ...
   - Update banner: rendered globally via `AppBarScaffold` (visible on all screens: home, settings, chat, contacts, calendar, event editor), not only on the home screen. The banner shows "Update verfügbar: vX.Y.Z" with a **Download** button and a dismiss button (session-scoped). Clicking Download starts the In-Network Update (§19.6.2); installation proceeds automatically after verification — no second confirmation step. Play Store installs are excluded — Android enforces signature consistency, so Play Store users receive updates through the store mechanism only (see §19.6.4).
...
|---|---|---|
| **Bootstrap** (early phase only) | All platforms, complete | Initial seed source that guarantees full fragment coverage while the network is still small. Updated automatically by `release-build.sh`: new binary replaces the running daemon (stop → deploy → restart), manifest is copied to `~/.cleona/update_manifest.json`, then `publish-in-network-update.sh` seeds fragments into the fresh daemon. **Post-seed verification gate (V3.1.149):** after seeding, `release-build.sh` queries `get_seeded_platforms` via IPC and verifies that each built platform appears at the release version with correct binary size — mismatch or missing platform is a hard failure that aborts the release before GitHub publish. Minimum precondition: at least one reachable seeder (bootstrap or local daemon) with all built platforms verified. If bootstrap SSH fails, the local daemon's seeded state is checked as fallback; if neither is verified, the release is blocked. Once enough regular nodes hold fragments via distributed erasure coding (the primary distribution vector), the bootstrap's role becomes redundant. |
...
`scripts/deploy-macos-app.sh` baut das endgueltige `Cleona.app` Bundle zusammen: Flutter GUI + Daemon + dylibs in `Contents/Frameworks/`.
