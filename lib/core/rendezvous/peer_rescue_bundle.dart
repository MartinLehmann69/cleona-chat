...
class PeerRescueBundle {
  ...
  static PeerRescueBundleParseResult _parsePayload(
    ...
    try {
      ...
      // ── S368: THE VERSION BYTE IS NOW ALSO CHECKED ───────────
      //
      // It was read here, passed into `PeerRescueBundleParseResult`
      // and afterwards consulted by NOBODY. A bundle with a foreign
      // first byte thus ran through the whole parser as if it were
      // one of ours — the remaining fields were interpreted according to OUR structure,
      // although the byte had just said that it is a different one.
      //
      // This is not a compatibility place that falls, but a
      // check that was missing. `currentVersion` is a FORMAT BYTE (0x01)
      // and stays — it names the structure of this record, not the
      // application.
      final version = body[off++];
      if (version != currentVersion) {
        return PeerRescueBundleParseResult(
            networkTagValid: true,
            errorMessage: 'Unknown bundle version $version '
                '(expected $currentVersion) — not interpreted');
      }
