...
class ReedSolomon {
  ...
  // Process log: ReedSolomon is a process-wide singleton without
  // identity reference (the Cauchy matrix is the same for all
  // identities). Resolving the singleton per identity is not worth it
  // — the two lines below are configuration info and debug, the real
  // error cases run via `throw StateError(...)` (see the callers at
  // line ~81/116/157/300/352) directly to the caller with its own
  // identity-bound logger.
  final CLogger _log = CLogger.get('erasure', profileDir: AppPaths.dataDir);
